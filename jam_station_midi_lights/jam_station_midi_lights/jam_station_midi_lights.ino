...
void noteOn(byte channel, byte pitch, byte velocity) {
  ...
  // Drum or Snare
  // Dum
  if (channel == drum && pitch == 100) {
    for (int i = 0; i < 8; i ++) {
      leds[i] = CHSV(160, 255, 255);
    }
  }
  // Snare
  if (channel == drum && pitch == 101) {
    for (int i = 0; i < 8; i ++) {
  ...
  if (channel == same_time) {
    midi_effects_register = 1;
    midi_effects_counter += 1;
    midi_effects();
  }

  if (channel == same_note_time) {
    midi_effects_register = 1;
    midi_effects_counter += 1;
    midi_effects();
  }
  ...
  if (channel == same_division) {
    midi_effects_register = 1;
    midi_effects_counter += 4;
    midi_effects();
  }

  if (channel == same_melody) {
    midi_effects_register = 1;
